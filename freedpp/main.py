...
from freedpp.env.reward import Reward, MolLogP, PatternFilter, OutOfRange, identity, alzh, sklrz, prksn, cancer, resist, dyslipid, Brenk, SA, qed_mol
...
def init_rewards(args):
    ...
    costs = {
        'Ic50_park': Reward(prksn, OutOfRange(lower=0, upper=0.3, hard=hard), preprocess=preprocess),
        'Ic50_sklr': Reward(sklrz, OutOfRange(lower=0.7, upper=1, hard=hard), preprocess=preprocess),
        'Ic50_alzh': Reward(alzh, OutOfRange(lower=0, upper=0.3, hard=hard), preprocess=preprocess),
        'Ic50_lung': Reward(cancer, OutOfRange(lower=0.7, upper=1, hard=hard), preprocess=preprocess),
        'Ic50_resist': Reward(resist, OutOfRange(lower=0, upper=0.3, hard=hard), preprocess=preprocess),
        'Ic50_dslp': Reward(dyslipid, OutOfRange(lower=0, upper=0.3, hard=hard), preprocess=preprocess),
